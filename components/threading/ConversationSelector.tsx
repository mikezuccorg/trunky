...
export function ConversationSelector({ threads, mainThreadId, onNavigate, onClose }: ConversationSelectorProps) {
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                <div className={`flex-shrink-0 mt-0.5 ${isCurrentRoot ? 'text-white' : 'text-text-secondary'}`}>
                  <FolderOpen size={16} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className={`text-[10px] flex-shrink-0 ml-2 ${isCurrentRoot ? 'opacity-90' : 'text-text-secondary'}`}>
                      {formatTimestamp(rootThread.createdAt)}
                    </div>
                  </div>

                  <div className={`text-xs mb-2 line-clamp-2 ${isCurrentRoot ? 'opacity-90' : 'text-text-secondary'}`}>
                    {preview}
                  </div>

                  <div className={`flex items-center gap-3 text-[10px] ${isCurrentRoot ? 'opacity-80' : 'text-text-secondary'}`}>
